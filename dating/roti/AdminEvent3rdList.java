...
/**
 * Servlet implementation class AdminEvent3rdReview
 */
@WebServlet("/zhs/admin/shenpiliebiao")
public class AdminEvent3rdList extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public AdminEvent3rdList() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try{
			...
			if(isAdmin){
				...
				RequestDispatcher dispatcher = getServletContext().getRequestDispatcher("/zhs/admin/admin_3rd_event_edit_list.jsp");
	...
	private HttpServletRequest getAllArticles(Connection con, HttpServletRequest request) throws SQLException{
		...
		/**
		 * 
		 *  posttype 2 - get all posts from all users
		 *  poststatus 1 - post which is not in draft
		 *  
		 */
		String sql_article_genre = "select * from post where poststatus = 1 and posttype = 2 ORDER BY PostCreatedTime DESC";
		...
		if(rm.first()){
			...
			while(isLast) {
				...
				if(rm.next()){
					...
					article.setVerify(rm.getInt("verify"));
