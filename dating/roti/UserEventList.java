 ...
 */
@WebServlet("/zhs/yonghu/liebiao")
public class UserEventList extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try{
			...
			PreparedStatement sql_is_user = con.prepareStatement("select email, role, uuid, status, userid from hunyin.user where email = ? and uuid = ? and role = ? and status = ?");
			...
			if(isUser){
				...
				RequestDispatcher dispatcher = getServletContext().getRequestDispatcher("/zhs/yonghu/user_edit_list.jsp");
	...
	private HttpServletRequest getAllArticles(Connection con, HttpServletRequest request, int userid) throws SQLException{
		...
		String sql_article_genre = "select * from post where posttype = 2 and poststatus = 1 and userid = ? ORDER BY PostCreatedTime DESC";
