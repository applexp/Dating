...
public class AdminEventList extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try{
			...
			if(isAdmin){
				...
				RequestDispatcher dispatcher = getServletContext().getRequestDispatcher("/zhs/admin/admin_edit_list.jsp");
	...
	private HttpServletRequest getAllArticles(Connection con, HttpServletRequest request, int posttype, String genre) throws SQLException{
		...
		if(genre.equals("a")){
			...
			sql_article_genre = "select * from post where poststatus = 1 and posttype = 1 and tag = 'wansheng' ORDER BY PostCreatedTime DESC";
		}else{
